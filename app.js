/* GolfTrack PWA - app.js (favorites removed, styling kept consistent; overlay & edit fixes retained) */
...
const DEFAULTS = {
  ...
  favorites: [],              // keep empty; favorites UI is disabled
  penaltyTypes: [
...
const quickFavorites = $('quickFavorites'); // present in DOM, but hidden & unused

let CURRENT_ROUND = null;

// --- Models ---
function createHole(number, par = 4) {
  return { id: uid(), number, par, shots: [], putts: 0, penalties: [] };
}
function createRound(course, date, viewMode = 'all') {
  return { id: uid(), date, course, holes: [], notes: '', createdAt: new Date().toISOString(), currentHole: 0, viewMode };
}
function createShot(club, strokeType, outcome, lie = '', slope = '', notes = '') {
  return { id: uid(), club, strokeType, lie, slope, outcome, notes, ts: new Date().toISOString() };
}
function createPenalty(type, strokes = 1, note = '') {
  return { id: uid(), type, strokes, note, ts: new Date().toISOString() };
}

// Local course
const LOCAL_COURSE = {
  name: 'Local Course',
  pars: [4,3,4,3,5,4,5,4,4, 4,4,5,3,4,5,4,3,4]
};

// Helpers
function getActiveHole(round){
  if(!round || !Array.isArray(round.holes) || !round.holes.length) return null;
...
function computeHoleStrokes(hole){
  ...
  const penalties = (hole.penalties||[]).reduce((acc,p)=>acc + (p.strokes||1), 0);
...
// Approx FIR/GIR (see earlier note)
...
function computeGIR(hole){
  ...
  const shots = hole.shots || [];
  const nonPuttShots = shots.length - shots.filter(s => (s.strokeType||'').toLowerCase() === 'putt').length;
...
async function createLocalCourseRound(){
  ...
  for(let i=0;i<LOCAL_COURSE.pars.length;i++) r.holes.push(createHole(i+1, LOCAL_COURSE.pars[i]));
  await saveRound(r);
  CURRENT_ROUND = r;
  renderActiveRound();
  renderRoundsList();
...
$('navRounds').addEventListener('click', ()=> { /* keep default main view */ });
  ...
  rounds.forEach(r=>{
    ...
    el.innerHTML = `<div class="row between">
      ...
      <div style="display:flex;flex-direction:column;gap:8px">
        ...
        <button class="btn danger small" data-id="${r.id}" data-action="delete">Delete</button>
      </div>
    </div>`;
...
function renderActiveRound(){
  ...
  roundMeta.innerHTML = `Date ${CURRENT_ROUND.date} • ${CURRENT_ROUND.holes.length} hole(s) • Total: ${totals.totalStrokes} (Par ${totals.totalPar}) • ${totals.diff>0? '+'+totals.diff : (totals.diff<0? totals.diff : 'E')}
    <div style="margin-top:6px" class="row gap">
      <button id="toggleViewBtn" class="btn small" style="padding:8px;font-size:13px">${viewToggleText}</button>
      <button id="viewScorecardBtn" class="btn small" style="padding:8px;font-size:13px">View Scorecard</button>
    </div>`;
  ...
  // HIDE favorites strip if present in DOM (legacy markup)
  if (quickFavorites) {
    quickFavorites.style.display = 'none';
  ...
  if(CURRENT_ROUND.viewMode === 'current'){
    ...
    col.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center">
      ...
      <div style="text-align:right">
        <div style="font-weight:800;font-size:20px">${strokes}</div>
    ...
    (CURRENT_ROUND.holes || []).forEach((h, idx)=>{
      ...
      col.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center">
        <div><strong>Hole ${h.number}</strong> <div class="muted small">Par ${h.par||'-'}</div></div>
        <div style="text-align:right">
          <div style="font-weight:800">${strokes}</div>
          <div class="muted small">${res}</div>
        </div>
      </div>
      <div class="muted small" style="margin-top:6px">${(h.shots||[]).length} shots • Putts: ${h.putts||0} • Penalties: ${penCount}</div>
      <div style="margin-top:8px">
        <button class="btn small" style="padding:8px;font-size:13px" data-r="${CURRENT_ROUND.id}" data-h="${h.id}" onclick="openDetailedShotFormById(this)">Add Shot</button>
      </div>`;
  ...
  summary.innerHTML = `<div class="row between">
    ...
    <div style="text-align:right"><div style="font-weight:800;font-size:18px">${totals.totalStrokes}</div><div class="muted small">Par ${totals.totalPar} • ${totals.diff>0? '+'+totals.diff : (totals.diff<0? totals.diff : 'E')}</div></div>
  </div>`;
  recentShots.appendChild(summary);
}

// Open detailed form (helper ID hook)
...
// Favorites quick log (kept for compatibility but not surfaced)
async function quickLogFromFavorite(){ alert('Quick favorites have been removed. Use “Add Shot”.'); }
...
function openDetailedShotForm(round,hole){
  ...
  if(isFirstShot) {
    recommendedClubs = par <= 3 ? ['6-iron','7-iron','8-iron','9-iron','PW','GW']
  ...
  const existingShotsHTML = (hole.shots && hole.shots.length) ? 
    `<div style="margin-bottom:16px">
      ...
      <div id="existingShots" style="max-height:200px;overflow-y:auto;border:1px solid var(--border);border-radius:8px;padding:8px">
        ${hole.shots.map((shot) => `
          <div class="shot-item" style="display:flex;justify-content:space-between;align-items:center;padding:6px;border-bottom:1px solid var(--border);cursor:pointer" data-shot-id="${shot.id}">
            <div>
            ...
            <div class="row">
              ...
              <button class="btn small danger" onclick="deleteShot('${shot.id}')" style="padding:4px 8px;font-size:12px">×</button>
            </div>
          </div>
        `).join('')}
      </div>
  ...
  form.innerHTML = `<h3>Shot — Hole ${hole.number} (Par ${par})</h3>
    ...
    <div>
      ...
      <div class="row" style="gap:8px;align-items:center">
        <select id="penaltySelect" class="input" style="flex:1">${penaltyOptionsHTML}</select>
        <input id="penaltyNote" class="input" placeholder="Note (optional)" style="width:160px"/>
        <button id="addPenaltyBtn" class="btn small">Add</button>
      </div>
  ...
  if(!isFirstShot) {
    ...
    if(!hasPenalties) availableLies = CFG.lies.filter(lie => lie !== 'Tee');
  }
  ...
  if(isFirstShot) {
    setTimeout(() => {
      const teeBtn = Array.from(dlie.querySelectorAll('button')).find(b=>b.textContent==='Tee');
      if(teeBtn){
        select('lie','Tee',teeBtn);
        dlie.querySelectorAll('button').forEach(btn => { btn.style.pointerEvents = 'none'; btn.style.opacity = '0.55'; });
        teeBtn.style.pointerEvents='auto'; teeBtn.style.opacity='1';
      }
      const flatBtn = Array.from(dslope.querySelectorAll('button')).find(b=>b.textContent==='Flat');
      if(flatBtn){
        select('slope','Flat',flatBtn);
        dslope.querySelectorAll('button').forEach(btn => { btn.style.pointerEvents = 'none'; btn.style.opacity = '0.55'; });
        flatBtn.style.pointerEvents='auto'; flatBtn.style.opacity='1';
      }
    }, 0);
  }
  ...
  function selectMultiple(k,v,btn){
    ...
    if(index > -1) { selection[k].splice(index, 1); btn.classList.remove('sel'); }
    else { selection[k].push(v); btn.classList.add('sel'); }
  }
  ...
  form.querySelector('#lostBall').onclick = async ()=>{
    if(!CFG.pgaLostBall){
      if(!confirm('PGA lost-ball handling is disabled in Settings. Add penalty anyway?')) return;
    }
    ...
    await saveRound(round);
    alert('Lost Ball recorded: +1 penalty stroke (stroke-and-distance). Replay from previous spot as required by the rules.');
    overlay.classList.add('hidden'); overlay.innerHTML=''; renderActiveRound();
  };
  ...
  window.editShot = async function(shotId) {
    ...
    const clubBtn  = findBtnByText(dclub,  shot.club);  if (clubBtn) select('club', shot.club, clubBtn);
    const strokeBtn= findBtnByText(dstroke, shot.strokeType); if (strokeBtn) select('stroke', shot.strokeType, strokeBtn);
    const lieBtn   = findBtnByText(dlie,   shot.lie);   if (lieBtn) select('lie', shot.lie, lieBtn);
    const slopeBtn = findBtnByText(dslope, shot.slope); if (slopeBtn) select('slope', shot.slope, slopeBtn);

    const outcomes = Array.isArray(shot.outcome) ? shot.outcome
...
function openSettings(){
  ...
  form.innerHTML = `<h3>Settings</h3>
    <div class="smallNote">Edit your club, lies, slopes, and penalty types. Quick favorites were removed—use “Add Shot”.</div>
    <div style="height:8px"></div>

    <label class="small">Clubs (comma separated)</label>
    <textarea id="clubsText" class="input" style="width:100%;min-height:60px;border-radius:10px;padding:8px">${cfg.clubs.join(', ')}</textarea>

    <div style="height:8px"></div>
    <label class="small">Lies (comma separated)</label>
    <textarea id="liesText" class="input" style="width:100%;min-height:60px;border-radius:10px;padding:8px">${cfg.lies.join(', ')}</textarea>

    <div style="height:8px"></div>
    <label class="small">Slopes / Lie types (comma separated)</label>
    <textarea id="slopesText" class="input" style="width:100%;min-height:60px;border-radius:10px;padding:8px">${cfg.slopes.join(', ')}</textarea>

    <div style="height:8px"></div>
    <label class="small">Penalty Types (one per line, format: key | label | strokes)</label>
    <textarea id="penText" class="input" style="width:100%;min-height:120px;border-radius:10px;padding:8px">${(cfg.penaltyTypes||[]).map(p=>`${p.key}|${p.label}|${p.strokes||1}`).join('\n')}</textarea>
